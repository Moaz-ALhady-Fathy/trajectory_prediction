...
# inputs and outputs
parser.add_argument("prepropath", type=str) #used
parser.add_argument("outbasepath", type=str,
                    help="full path will be outbasepath/modelname/runId") # packed_models/
parser.add_argument("modelname", type=str)#best_simaug_model
parser.add_argument("--runId", type=int, default=0,
                    help="used for run the same model multiple times") #used

# ---- gpu stuff. Now only one gpu is used
parser.add_argument("--gpuid", default=0, type=int) #used

parser.add_argument("--load", action="store_true",
                    default=False, help="whether to load existing model")
parser.add_argument("--load_best", action="store_true",
                    default=False, help="whether to load the best model") #used
# use for pre-trained model
parser.add_argument("--load_from", type=str, default=None) 

parser.add_argument("--save_output", default=None) #used

# ------------- experiment settings
parser.add_argument("--obs_len", type=int, default=8) #used
parser.add_argument("--pred_len", type=int, default=12) #used
...
parser.add_argument("--emb_size", type=int, default=128) #used 32 
parser.add_argument("--enc_hidden_size", type=int,
                    default=256, help="hidden size for rnn") #used 
...
# --------- scene features
parser.add_argument("--scene_conv_kernel", default=3, type=int) #used 
parser.add_argument("--scene_h", default=36, type=int) #used 
parser.add_argument("--scene_w", default=64, type=int) #used 
parser.add_argument("--scene_class", default=11, type=int)
parser.add_argument("--scene_conv_dim", default=64, type=int)#used 
parser.add_argument("--pool_scale_idx", default=0, type=int)
parser.add_argument("--convlstm_kernel", default=3, type=int)

parser.add_argument("--scene_grid_strides", default="4,8,16")#used 2,4
parser.add_argument("--use_grids", default="1,1,1") #used 1,0
parser.add_argument("--val_grid_num", type=int, default=1,
                    help="which grid to use for validation metric")#used 0
...
parser.add_argument("--use_gnn", action="store_true")#used
...
parser.add_argument("--grid_loss_weight", default=1.0, type=float)#used
parser.add_argument("--grid_reg_loss_weight", default=1.0, type=float)#used .5

# ---------------------------- training hparam
parser.add_argument("--save_period", type=int, default=300,
                    help="num steps to save model and eval")#used 3000
parser.add_argument("--batch_size", type=int, default=64)#used 12
# num_step will be num_example/batch_size * epoch
parser.add_argument("--num_epochs", type=int, default=100)#used 30
# drop out rate
parser.add_argument("--keep_prob", default=1.0, type=float,
                    help="1.0 - drop out rate")#used
# l2 weight decay rate
parser.add_argument("--wd", default=0.0001, type=float,
                    help="l2 weight decay loss")#used 0.001
...
parser.add_argument("--learning_rate_decay", default=0.95,
                    type=float, help="learning rate decay")#used
parser.add_argument("--num_epoch_per_decay", default=2.0,
                    type=float, help="how epoch after which lr decay")#used 5
parser.add_argument("--init_lr", default=0.2, type=float,
                    help="Start learning rate")#used 0.3
